...
import { DollarSign, Banknote, ArrowRight, X, Loader2 } from 'lucide-react';
import type { Employee, Supplier } from '../lib/types';

type CashRegisterExpensesProps = {
  registerId: string;
  onClose: () => void;
  onSuccess: () => void;
};

export function CashRegisterExpenses({ registerId, onClose, onSuccess }: CashRegisterExpensesProps) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="mt-1 relative rounded-md shadow-sm">
              ...
              <input
                ...
                className="block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-150"
                placeholder="0.00"
              />
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-150"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={loading}
              className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 transition-all duration-150 transform hover:scale-105"
            >
              {loading ? (
